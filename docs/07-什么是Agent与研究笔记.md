...
这个定义不是我拍脑袋想的，是被 21 篇 postmortem 一篇一篇逼出来的。最早我以为"Agent = LLM + 工具调用"，后来被"AI 在交付里形式合规地撒谎"打脸（PM-003）；加上"门禁形式通过但语义漏项"（PM-018）、"AI 越权裁决业务语义"（PM-010）、"raw data 溜进云端"（数据边界事故）这几次之后，我才意识到：**会想、会写、会调用工具，只是 Agent 的"能力面"；真正让一个 Agent 能在工业现场上岗的，是它的"治理面"和"运行态面"。**
...
2. **判例驱动生长**：新类型必须由真实案例逼出来，且新增时必须回答三个问题——这个类型的事实由谁有权写入？需要什么证据支撑？什么条件下会失效？回答不了就不许进。
...
下面五条机制，每一条都对应 demands_agent 里被真实事故逼出来的实现。我把它们写成"问题 → 机制 → 反例"的形式，方便迁移到别的 Agent 项目。
...
想看这些机制怎么在一个真实需求里串起来，回到 [[02-工程系统全景]]。想看它们怎么被事故逼出来的，去 [[04-真实复盘]]。
