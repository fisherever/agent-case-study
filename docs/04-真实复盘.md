...
> 为什么读这篇：前一篇的 10 条法则都是"结论"，这篇是"结论怎么被逼出来的"。
> 读完带走：5 个最有讲头的事故，每个讲透现象→根因→修复→法则映射。

## 先看全局：21 篇 postmortem 的分布

demands_agent 有 21 篇结构化 postmortem（PM-001~021）。**这个数字本身就是一种成熟度证据**——绝大多数个人项目拿不出 5 篇像样的复盘，更别说 21 篇全部按统一模板（现象/根因/修复/Agent 指令）结构化。
...
我从 21 篇里选了 5 篇最有讲头的——**选择标准是：每篇都逼出了一条独立法则，且故事本身有普遍启示。**
...
这次事故最大的教训不是"漏了 Hive DDL"，而是**重新定义了门禁的角色认知**：
...
我在分析里给出结论："对某张表调 metadata lineage 提示'不在 lineage graph 中'，目前 Nebula 侧无血缘图信息，只能黑盒使用。"

用户通过 Nebula 前端确认：**该表在 Nebula 中确实有完整血缘**（包括任务、上下游任务和 SQL）。实际上 `demands-agent lineage search/job/sql` 也能拿到完整信息，只是当时没走这条路径。
...
PM-020 最精彩的部分是它**排除了五个表象归因**，收敛到单一根因：
...
这两篇 PM 逼出了 [[03-可迁移法则]] 法则 5 的循环检测器四问：
...
**这 21 篇 PM 是 demands_agent 最硬的成熟度证据。** 它们证明这个项目不是在 happy path 上跑出来的 demo——每一个 gate、每一条契约、每一层边界，背后都有一次真实事故。
